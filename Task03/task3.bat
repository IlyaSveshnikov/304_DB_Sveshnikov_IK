...
echo --------------------------------------------------
sqlite3 movies_rating.db -box -echo "select strftime('%Y', register_date) AS year, count(id) as num_users from users group by year order by year;"
sqlite3 movies_rating.db -box -echo "select count(id) as max_users, strftime('%Y', register_date) as year from users group by year order by max_users DESC limit 1;"
sqlite3 movies_rating.db -box -echo "select count(id) as min_users, strftime('%Y', register_date) as year from users group by year order by min_users limit 1;"
echo " "